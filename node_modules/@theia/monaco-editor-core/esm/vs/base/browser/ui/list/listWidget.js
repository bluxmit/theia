...
const aria_1 = require("../aria/aria");
...
class KeyboardController {
    ...
    get onKeyDown() {
        return event_2.Event.chain(this.disposables.add(new event_1.DomEmitter(this.view.domNode, 'keydown')).event)
...
class TypeLabelController {
    ...
    enable() {
        ...
        const onChar = event_2.Event.chain(this.enabledDisposables.add(new event_1.DomEmitter(this.view.domNode, 'keydown')).event)
            .filter(e => !isInputElement(e.target))
            ...
            .event;
        const onClear = event_2.Event.debounce(onChar, () => null, 800);
        const onInput = event_2.Event.reduce(event_2.Event.any(onChar, onClear), (r, i) => i === null ? null : ((r || '') + i));
...
const defaultStyles = {
    ...
    tableOddRowsBackgroundColor: color_1.Color.fromHex('#cccccc').transparent(0.04)
};
...
class List {
    ...
    get onDidChangeFocus() {
        return event_2.Event.map(this.eventBufferer.wrapEvent(this.focus.onChange), e => this.toListEvent(e));
    }
    get onDidChangeSelection() {
        return event_2.Event.map(this.eventBufferer.wrapEvent(this.selection.onChange), e => this.toListEvent(e));
    }
    ...
    get onContextMenu() {
        ...
        const fromKeyDown = event_2.Event.chain(this.disposables.add(new event_1.DomEmitter(this.view.domNode, 'keydown')).event)
            .map(e => new keyboardEvent_1.StandardKeyboardEvent(e))
            .filter(e => didJustPressContextMenuKey = e.keyCode === 58 /* KeyCode.ContextMenu */ || (e.shiftKey && e.keyCode === 68 /* KeyCode.F10 */))
            .map(event_1.stopEvent)
            .filter(() => false)
            .event;
        const fromKeyUp = event_2.Event.chain(this.disposables.add(new event_1.DomEmitter(this.view.domNode, 'keyup')).event)
            ...
            .event;
        const fromMouse = event_2.Event.chain(this.view.onContextMenu)
