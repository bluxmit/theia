...
exports.GhostTextWidget = void 0;
...
let GhostTextWidget = class GhostTextWidget extends lifecycle_1.Disposable {
    constructor(editor, model, instantiationService, languageService) {
        ...
        this._register(this.editor.onDidChangeConfiguration((e) => {
            if (e.hasChanged(29 /* EditorOption.disableMonospaceOptimizations */)
                || e.hasChanged(105 /* EditorOption.stopRenderingLineAfter */)
                || e.hasChanged(88 /* EditorOption.renderWhitespace */)
                || e.hasChanged(83 /* EditorOption.renderControlCharacters */)
                || e.hasChanged(45 /* EditorOption.fontLigatures */)
                || e.hasChanged(44 /* EditorOption.fontInfo */)
                || e.hasChanged(59 /* EditorOption.lineHeight */)) {
                this.update();
            }
        }));
    ...
    update() {
        ...
        if (ghostText instanceof ghostText_1.GhostTextReplacement) {
            this.replacementDecoration.setDecorations([
                {
                    range: new range_1.Range(ghostText.lineNumber, ghostText.columnStart, ghostText.lineNumber, ghostText.columnStart + ghostText.length),
                    options: {
        ...
        if (0 < 0) {
            ...
            this.viewMoreContentWidget = this.renderViewMoreLines(new position_1.Position(ghostText.lineNumber, this.editor.getModel().getLineMaxColumn(ghostText.lineNumber)), '', 0);
        }
        else {
            (_a = this.viewMoreContentWidget) === null || _a === void 0 ? void 0 : _a.dispose();
            this.viewMoreContentWidget = undefined;
    ...
    setParts(lineNumber, parts, hiddenText) {
        ...
        if (hiddenText) {
            hiddenTextDecorations.push({
                range: range_1.Range.fromPositions(new position_1.Position(lineNumber, hiddenText.column), new position_1.Position(lineNumber, hiddenText.column + hiddenText.length)),
                options: {
        ...
        this.editor.changeDecorations(accessor => {
            this.decorationIds = accessor.deltaDecorations(this.decorationIds, parts.map(p => {
                return ({
                    range: range_1.Range.fromPositions(new position_1.Position(lineNumber, p.column)),
                    options: {
...
function renderLines(domNode, tabSize, lines, opts, languageIdCodec) {
    ...
    const stopRenderingLineAfter = opts.get(105 /* EditorOption.stopRenderingLineAfter */);
    // To avoid visual confusion, we don't want to render visible whitespace
    const renderWhitespace = 'none';
    const renderControlCharacters = opts.get(83 /* EditorOption.renderControlCharacters */);
    const fontLigatures = opts.get(45 /* EditorOption.fontLigatures */);
    const fontInfo = opts.get(44 /* EditorOption.fontInfo */);
    const lineHeight = opts.get(59 /* EditorOption.lineHeight */);
...
class ViewMoreLinesContentWidget extends lifecycle_1.Disposable {
...
(0, themeService_1.registerThemingParticipant)((theme, collector) => {
    const foreground = theme.getColor(editorColorRegistry_1.ghostTextForeground);
    if (foreground) {
    ...
    const background = theme.getColor(editorColorRegistry_1.ghostTextBackground);
    if (background) {
    ...
    const border = theme.getColor(editorColorRegistry_1.ghostTextBorder);
    if (border) {
