...
const snippetParser_1 = require("./snippetParser");
const snippetVariables_1 = require("./snippetVariables");
class OneSnippet {
    ...
    getEnclosingRange() {
        ...
        for (const decorationId of this._placeholderDecorations.values()) {
            const placeholderRange = (0, types_1.withNullAsUndefined)(model.getDecorationRange(decorationId));
            if (!result) {
...
let SnippetSession = class SnippetSession {
    constructor(editor, template, options = _defaultOptions, _languageConfigurationService) {
    ...
    static createEditsAndSnippets(editor, template, overwriteBefore, overwriteAfter, enforceFinalTabstop, adjustWhitespace, clipboardText, overtypingCapturer, languageConfigurationService) {
        const edits = [];
        const snippets = [];
        if (!editor.hasModel()) {
        ...
        for (const { selection, idx } of indexedSelections) {
            ...
            snippet.resolveVariables(new snippetVariables_1.CompositeSnippetVariableResolver([
                ...
                new snippetVariables_1.ClipboardBasedVariableResolver(readClipboardText, idx, indexedSelections.length, editor.getOption(71 /* EditorOption.multiCursorPaste */) === 'spread'),
